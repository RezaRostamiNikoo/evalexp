 ...
 */
export function memoize(fn, { hasher, limit } = { hasher: undefined, limit: undefined }) {
  limit = limit == null ? Number.POSITIVE_INFINITY : limit
  hasher = hasher == null ? JSON.stringify : hasher

  return function memoize() {
    if (typeof (memoize as any).cache !== 'object') {
    ...
    if ((memoize as any).cache.values.has(hash)) {
      (memoize as any).cache.lru.hit(hash)
      return (memoize as any).cache.values.get(hash)
    }

    const newVal = fn.apply(fn, args)
      (memoize as any).cache.values.set(hash, newVal)
      (memoize as any).cache.values.delete((memoize as any).cache.lru.hit(hash))
