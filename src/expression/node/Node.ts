...
export class Node {
   ...
   *                variables.
   */
  compile(): { evaluate: (scppe: Object) => any } {
   ...
   */
  _compile(math: Object, argNames: Object) {
   ...
   */
  forEach(callback: (child: Node, path: string, parent: Node) => void) {
   ...
   */
  map(callback: (child: Node, path: string, parent: Node) => Node) {
    // must be implemented by each of the Node implementations
   ...
   */
  traverse(callback: (node: Node, path: string | null, parent: Node | null) => void) {
